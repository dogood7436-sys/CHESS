...
- 기술 카드: 새 게임마다 각 플레이어가 10종의 일회성 기술 카드 중 하나를 랜덤으로 받습니다. 모든 카드는 UI에 `패시브` 또는 `액티브`가 표시됩니다. 부활 대기 말을 직접 추가하는 카드는 제거되었고, 예시로 폰이 한 번 정면의 상대 말을 잡는 패시브 카드, 다음 포획 보너스, 캐슬링 보너스, 부활 카운터 정화/봉인, 포인트 획득/흡수 카드 등이 있습니다.
- 사운드: HTML 버전은 이동/포획/부활/카드 발동 효과음을 WebAudio로 재생하며, `사운드 켜기` 이후 총 보유 포인트가 증가할수록 잔잔한 피아노 BGM이 더 빠르고 박진감 있게 바뀝니다.
